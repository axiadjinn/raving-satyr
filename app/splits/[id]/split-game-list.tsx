...
function SplitGameList({ splitGames, onUpdate }: { splitGames: SplitGame[], onUpdate: (splitGames: SplitGame[]) => void }) {
  function UpdateSplitGameInfo(splitGame: SplitGame) {
    onUpdate(
      splitGames.toSpliced(
        splitGames.findIndex(sg => sg.id == splitGame.id), 1, splitGame
      )
    );
  }
