      ...
      <Box>
        <Heading as="h4" size="sm" py={2}>Win-at...</Heading>
        {
          levelChecks.map(level => (
            <Flex w="100%" pb={1}>
              <Text display="inline" pe={1}>Level {level}? </Text>
              <Spacer />
              <Box display="inline" px={2}>
                <ThreeStateSelection selection={ earlyGamePlanning[`winlvl${level}` as keyof EarlyGamePlanning] } onStateChange={(state) => { onWinUpdate(state, level) }} />
              </Box>
            </Flex>
          ))
        }
